...
class Machine {
    constructor() {
        this.machine = [[], [], [], [], [], []];
        this.head = [HEAD_START, HEAD_START, HEAD_START, HEAD_START, HEAD_START, HEAD_START];
        this.state = "";
        ...
        this.konvaHandle = new KonvaController();
    }
    ...
    initInput(input) {
        ...
        for (const i in input) {
            ...
            this.konvaHandle.pushLeft(input[input.length - i], 1);
        }
    ...
    prepareToRun(rule, input, name) {
        ...
        this.konvaHandle.drawTuringPlane(numTuringType);
    }
    ...
    run() {
        ...
        var timer = setInterval(() => {
            ...
            for (const i in this.rule) {
                if (this.rule[i].stateFrom == this.state) {
                    for (const j in this.rule[i].read) {
                        if (
                            (this.rule[i].read[j] == "_" && this.machine[j][this.head[j]] == null) ||
                            (this.rule[i].read[j] == "_" && this.machine[j][this.head[j]] == "") ||
                    ...
                    if (correctRead) {
                        ...
                        for (const j in this.rule[i].update) {
                            ...
                            if (this.rule[i].move[j] == LEFT) {
                                ...
                                this.konvaHandle.pushLeft(update, parseInt(j) + 1);
                            } else if (this.rule[i].move[j] == RIGHT) {
                                this.pushRight(update, j);
                                this.konvaHandle.pushRight(update, parseInt(j) + 1);
                            } else {
                                this.pushStay(update, j);
                                this.konvaHandle.pushStay(update, parseInt(j) + 1);
                            }
                        }
            ...
            if (!running) {
                ...
                if (this.state != this.acceptState) console.log("Last State: " + this.state);
                console.log("Output:");
                var formattedMachineOutput = [];
                for (const i in this.machine) {
                    if (this.machine[i].length > 0) {
                        console.log("[" + i + "]:", this.machine[i].join("").split("").join(""));
                        formattedMachineOutput.push(this.machine[i].join("").split("").join(""));
                    }
                }
                turingDoneHandler(this.state == this.acceptState, formattedMachineOutput);
                clearInterval(timer);
            }
        }, interval);
    }
