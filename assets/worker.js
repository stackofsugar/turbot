...
function translateTemperatur(num, symbolPos, symbolNeg) {
    var intNum = parseInt(num);
    console.log(intNum);
    if (intNum > 0) {
        return repeat(symbolPos, intNum);
    } else if (!intNum) {
        return String("");
    } else {
        return repeat(symbolNeg, -intNum);
    }
}
...
class Operation {
    ...
    static prepareMachine() {
        ...
        } else if (this.operationName == "temperatur") {
            if (this.turingType == "4-tapes C -> K") {
                this.rule = temperatur_ck;
            } else if (this.turingType == "4-tapes K -> C") {
                this.rule = temperatur_kc;
            } else if (this.turingType == "6-tapes C -> F") {
                this.rule = temperatur_cf;
            } else if (this.turingType == "6-tapes F -> C") {
                this.rule = temperatur_fc;
            } else if (this.turingType == "6-tapes F -> K") {
                this.rule = temperatur_fk;
            } else if (this.turingType == "6-tapes K -> F") {
                this.rule = temperatur_kf;
            }
        } else {
    ...
    static prepareInput() {
        if (equals(this.operationName, ["temperatur", "logaritma", "faktorial"])) {
        ...
        if (this.operationName == "logaritma") {
            ...
            $("#operationLabelHeading").text(toTitleCase(this.operationName));
        }

        for (const prop in operationSymbolCastings) {
            if (this.operationName == prop) {
                $("#operationSign").text(operationSymbolCastings[prop]);
            }
        }
    }
    ...
    static translateSingleOperation() {
        if (this.operationName == "faktorial") {
            ...
            if (this.turingType == "4-tapes C -> K") {
                this.outputField.val("000010001" + translateTemperatur(this.inputField.val(), "C", "D"));
            } else if (this.turingType == "4-tapes K -> C") {
                this.outputField.val("000010001" + translateTemperatur(this.inputField.val(), "K", "L"));
            } else if (this.turingType == "6-tapes C -> F") {
                this.outputField.val("00000100001" + translateTemperatur(this.inputField.val(), "C", "D"));
            } else if (this.turingType == "6-tapes F -> C") {
                this.outputField.val("00000100001" + translateTemperatur(this.inputField.val(), "F", "G"));
            } else if (this.turingType == "6-tapes F -> K") {
                this.outputField.val("00000100001" + translateTemperatur(this.inputField.val(), "F", "G"));
            } else if (this.turingType == "6-tapes K -> F") {
                this.outputField.val("00000100001" + translateTemperatur(this.inputField.val(), "K", "L"));
            }
        }
    }
    ...
    static prepare() {
        if (this.rule == "") {
            ...
            $("#turingStartButton").removeClass("d-none");
        }
    }

    static run() {
        $("#turingStartButton").addClass("disabled");
        this.machine.run();
    }

    static handleDone(accepted, output) {
        $("#turingStartButton").addClass("d-none");
        if (accepted == true) {
            $("#turingResultRejected").addClass("d-none");
        } else {
            $("#turingResultAccepted").addClass("d-none");
        }
